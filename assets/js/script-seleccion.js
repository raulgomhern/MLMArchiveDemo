document.addEventListener('DOMContentLoaded', () => {
  // Reparto de vocabulario entre varios proyectos (sin meter todo en uno)
  const proyectos = [
    {
      id: "bordergames",
      nombre: "Bordergames",
      ...
      palabras: [
        "participación ciudadana","co-creación","inclusión","videojuego","comunidad","juventud","barrio","cultura digital",
        "Justicia social","Igualdad de oportunidades","Pensamiento crítico"
      ],
    },
    {
      id: "megafone",
      ...
      palabras: [
        "tecnologías cívicas","comunicación comunitaria","móviles","datos ciudadanos","derechos","visibilidad social",
        "Transformación digital","Ciencia ciudadana","Datos urbanos"
      ],
    },
    {
      id: "wikimap",
      ...
      palabras: [
        "mapas participativos","datos urbanos","memoria colectiva","interfaz web","participación vecinal",
        "Participación ciudadana","Laboratorio ciudadano"
      ],
    },
    {
      id: "pigcity",
      ...
      palabras: [
        "urbanismo especulativo","energía renovable","eficiencia","economía circular","agrociudad","comunicación visual",
        "Energías renovables","Eficiencia energética","Materiales sostenibles"
      ],
    },
    {
      id: "worldprocessor",
      ...
      palabras: [
        "visualización de datos","cartografía crítica","cambio climático","recursos","conciencia ambiental",
        "Calidad del aire","Contaminación acústica","ODS 11","Agenda 2030"
      ],
    },
    {
      id: "balcanes",
      ...
      palabras: [
        "videojuego crítico","participación","geopolítica","inmersión","educación crítica","cultura digital",
        "Verificación de información","Transformación digital"
      ],
    }
  ];
  ...
  // Recuperar selección múltiple
  let selected = [];
  try {
    selected = JSON.parse(localStorage.getItem('userKeywords') || '[]');
  } catch {
    selected = [];
  }
  ...
  // Filtrado por cualquiera de las palabras; si no hay selección, mostrar todo; si queda vacío, fallback a todo
  let filtrados = selectedLower.length
    ...
    : proyectos.slice();
  if (filtrados.length === 0) filtrados = proyectos.slice(); // fallback [web:41][web:42]

  // Mantener máximo 3 cards como pediste que se pueda elegir “una de las 3”
  const top3 = filtrados.slice(0, 3);

  renderCards(top3, userInfo, contenedor, imagen);

  // Selección de card única
  let activeId = null;
  contenedor.addEventListener('click', (e) => {
    const card = e.target.closest('.card');
    if (!card) return;
    document.querySelectorAll('.card').forEach(el => el.classList.remove('active')); // limpiar [web:59]
    card.classList.add('active');
    activeId = card.dataset.id;
    sendBtn.disabled = false;
  });

  // Enviar selección a la siguiente pantalla
  sendBtn.addEventListener('click', () => {
    if (!activeId) return;
    const proyecto = top3.find(p => p.id === activeId);
    if (!proyecto) return;

    // Preparar tres palabras clave para el texto final:
    // - Incluir hasta 2 de las seleccionadas por la usuaria si pertenecen al proyecto
    // - Completar hasta 3 con otras del proyecto si hiciera falta
    const seleccionadasEnProyecto = selected.filter(s =>
      proyecto.palabras.map(x => x.toLowerCase()).includes(s.toLowerCase())
    );
    const únicas = [...new Set(proyecto.palabras)];
    const claves = [
      ...seleccionadasEnProyecto.slice(0, 2),
      ...únicas.filter(k => !seleccionadasEnProyecto.some(s => s.toLowerCase() === k.toLowerCase())).slice(0, 3)
    ].slice(0, 3);

    // Guardar en localStorage lo necesario para la página 3
    ...
    // También conservar el userInfo original
...
function renderCards(lista, userInfo, contenedor, imagen) {
  contenedor.innerHTML = '';
  lista.forEach(p => {
    const card = document.createElement('div');
    card.className = 'card';
    card.dataset.id = p.id; // para identificar selección [web:64]
    card.innerHTML = `
      <img src="${imagen}" alt="${p.nombre}">
      <h3>${p.nombre}</h3>
      <p><strong>Autores:</strong> ${escapeHTML(p.autores)}</p>
      <p>${escapeHTML(p.descripcion)}</p>
      ${userInfo ? `<p><strong>Tu texto:</strong> ${sanitize(userInfo)}</p>` : ''}
      <div class="taglist">
